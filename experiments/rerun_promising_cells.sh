#!/usr/bin/env bash
# =============================================================================
# Targeted low-data reruns for specific (model, task) cells.
#
# This is the queue used to push up the weaker low-data cells in the main
# results table. It waits for a free GPU, skips cells whose results already
# exist, and logs each job. Adjust the run_job lines at the bottom to target
# other cells / hyperparameters.
# =============================================================================
set -u -o pipefail
...
WORK_BASE="${WORK_BASE:-${ROOT}/workspace/promising_reruns}"
RESULT_BASE="${RESULT_BASE:-${ROOT}/results/promising_reruns}"
LOG_DIR="${LOG_DIR:-${ROOT}/workspace/logs/promising_reruns}"
...
echo "[$(timestamp)] QUEUE COMPLETE failed=${FAILED}"
exit "$FAILED"
