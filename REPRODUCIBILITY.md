...
│   │   └── train_<model>_*.py    # original per-model scripts (legacy)
│   ├── evaluation/
│   │   ├── eval_unified.py       # canonical CLI evaluator (generates predictions)
│   │   └── eval_<model>_*.py     # original per-model scripts (legacy)
...
│   │   ├── session_mode_evaluation_labeling.py# RQ3 GPT labeling step
...
│   └── rerun_promising_cells.sh  # GPU-aware targeted low-data reruns
├── corpus/
...
`experiments/rerun_promising_cells.sh` waits for a free GPU, skips completed
cells, and reruns the weaker low-data cells with stronger recipes. Edit the
`run_job` lines at the bottom to target other cells.
...
numbers. Attention-map diagnostics are intentionally excluded from this
code-first artifact because the current paper uses appendix diagnostic data
rather than the newest unified training pipeline.
