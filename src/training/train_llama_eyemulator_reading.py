...
# --- Task Configuration ---
DATA_CATEGORY = "writing"  # Options: "reading", "writing"
TASK = "completion"       # Options: "translation", "summarization", "completion"

# --- Main Configuration ---
os.environ["CUDA_VISIBLE_DEVICES"] = "0"
LOG_FILE = f'training_log_{DATA_CATEGORY}.log'
logging.basicConfig(
    level=logging.INFO,
...
CATEGORY_DATA_FOLDER = f"./data_{DATA_CATEGORY}"

OUTPUT_DIR = f"./{MODEL_NAME}_advanced_final_{DATA_CATEGORY}_{TASK}"

TRAIN_FILE = os.path.join(BASE_DATA_FOLDER, f"{TASK}_train_final.jsonl")
VALID_FILE = os.path.join(BASE_DATA_FOLDER, f"{TASK}_valid_final.jsonl")
MAX_LENGTH = 1024
...
class LlamaForCausalLMWithWeightedLoss(LlamaForCausalLM):
    def forward(
        ...
        **kwargs,
        ...
        # This prevents the base model from calculating its own default loss.
...
def main():
    ...
    # Clear all global dictionaries to ensure a clean state for each run
    INDEX_TO_NGRAM.clear()
    NGRAM_COUNTS.clear()
    SEMANTIC_LABEL_ATTENTION.clear()
    SEMANTIC_ID_TO_LABEL.clear()
    
    logging.info(f"Using '{DATA_CATEGORY}' specific statistics with base training data.")
    
    # Define file paths
    indexed_ngrams_file = os.path.join(BASE_DATA_FOLDER, 'indexed_ngrams.json')
    monogram_counts_file = os.path.join(CATEGORY_DATA_FOLDER, f'{DATA_CATEGORY}_monogram_counts.json')
    bigram_counts_file = os.path.join(CATEGORY_DATA_FOLDER, f'{DATA_CATEGORY}_bigram_counts.json')
    trigram_counts_file = os.path.join(CATEGORY_DATA_FOLDER, f'{DATA_CATEGORY}_trigram_counts.json')
    beta_dist_file = os.path.join(CATEGORY_DATA_FOLDER, f'{DATA_CATEGORY}_beta_distribution.json')
    ...
    logging.info(f"Loading n-gram counts from: {CATEGORY_DATA_FOLDER}")
    ...
    with open(beta_dist_file, 'r') as f:
        ...
        for item in beta_data:
            label = item['semantic_label'].strip() # Use .strip() for robustness
    ...
    logging.info(f"Starting data-driven advanced training for '{DATA_CATEGORY}' on task '{TASK}'...")
    trainer.train()
