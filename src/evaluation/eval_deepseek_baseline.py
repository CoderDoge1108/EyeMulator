...
def generate_results(model_path, task, test_data_file, output_file, device):
    ...
    for i, example in enumerate(test_data):
        ...
        with torch.no_grad():
            # **FIX FOR REPETITIVE OUTPUT**
            # Using do_sample=True with temperature and top_p encourages varied output.
            # repetition_penalty > 1.0 penalizes the model for repeating words,
            # which is the most direct way to fix the looping issue.
            outputs = model.generate(
                **inputs,
                max_new_tokens=128,  # Summaries should be concise
        ...
        # Safely split the generated text to extract only the summary part
        if PROMPT_CONFIG['output_header'] in full_generated_text:
            ...
            # If the header is not in the output, it might be due to generation length
            # As a fallback, we take the part of the string after the prompt
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        # Using json.dump for a more standard JSON array output
        json.dump(results, f, indent=4)
    logging.info("Successfully generated and saved all results for the baseline model. 🎉")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate the baseline fine-tuned model.")
    ...
    # --- Paths for the BASELINE model ---
    MODEL_PATH = f"./deepseek_baseline_{args.task}"
    DATA_FILE = f"./data/{args.task}_valid_final.jsonl"
    OUTPUT_FILE = f"./deepseek_results/generated_results_baseline_{args.task}.json"
